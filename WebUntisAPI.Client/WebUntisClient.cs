...
public partial class WebUntisClient : IDisposable
{
    ...
    public async Task<SignInResult> SignInAsync(School school, string username, string password, string? mfaToken, string? clientName = null, CancellationToken ct = default)
    {
        ArgumentNullException.ThrowIfNull(school, nameof(school));

        return await SignInAsync(school.Server, school.LoginName, username, password, mfaToken, clientName, ct);
    }
    ...
    /// <exception cref="WebUntisException"></exception>
    /// <exception cref="ObjectDisposedException"></exception>
    public async Task<SignInResult> SignInAsync(string server, string loginName, string username, string password, string? mfaToken, string? clientName = null, CancellationToken ct = default)
    {
        // Check for disposing
        ...
        if (LoggedIn)
            throw new InvalidOperationException(string.Format("The client is already signed in. Use {0} to sign out.", nameof(SignOutAsync)));
